...
static int scull_p_open(struct inode *inode, struct file *filp)
{
    ...
    printk(KERN_NOTICE "open begin %d\n", scull_p_nr_devs);
...
int scull_p_init(void)
{
	...
	int i, result;
    dev_t firstdev;
   result = alloc_chrdev_region(&dev, scull_minor, scull_p_nr_devs,
				"pipee");
   scull_major = MAJOR(dev);
   // firstdev = MKDEV(scull_major, scull_minor);
	//result = register_chrdev_region(firstdev, scull_p_nr_devs, "pipee");
	if (result < 0) {
