...
ssize_t complete_read (struct file *filp, char __user *buf, size_t count, loff_t *pos)
{
	...
	wait_for_completion(&comp);//调用wait_for_completion(&comp)，进入睡眠，即等待completion “comp” 如果等待的completion发生了，complete_read函数将再次打印已被唤醒相关信息。
		...
		loff_t *pos)
{
	printk(KERN_DEBUG "process %i (%s) awakening the readers...\n",
			current->pid, current->comm);
	complete(&comp);//调用complete(&comp)触发completion事件，相应会唤醒一个在等待”comp”的进程。
